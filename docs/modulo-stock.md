...
Cada tarjeta mostrará la abreviatura de la unidad entre paréntesis junto al nombre del artículo. A la derecha indicará el resultado mediante un icono compacto: check verde para controlado, cruz roja para pendiente y guion gris para un artículo sin control periódico. El aviso de stock bajo, cuando corresponda, se mostrará debajo del icono. Al tocar o enfocar el icono se mostrará un tooltip con la fecha, el responsable y la antigüedad del control. La cabecera usará un layout sin elementos superpuestos para conservar legibles tanto el nombre como los indicadores y reducir la altura de las tarjetas sin marca.
